...
const NonAuthenticatedUserPage = ({onAuthenticationSuccess}: Props) => {
  ...
  return (
    <div className="flex items-center justify-center">
      <div className="p-2 rounded-lg border-orange-100 border-4 w-fit">
        <div className="flex flex-row justify-start">
          <button
            onClick={() => setAuthenticationTab(AuthenticationTab.LOGIN)}
            className={styles.get(AuthenticationTab.LOGIN)}
          >
            {AuthenticationTab.LOGIN}
          </button>
          <button
            onClick={() => setAuthenticationTab(AuthenticationTab.SIGN_UP)}
            className={styles.get(AuthenticationTab.SIGN_UP)}
          >
            {AuthenticationTab.SIGN_UP}
          </button>
          <button
            onClick={() =>
              setAuthenticationTab(AuthenticationTab.RESET_PASSWORD)
            }
            className={styles.get(AuthenticationTab.RESET_PASSWORD)}
          >
            {AuthenticationTab.RESET_PASSWORD}
          </button>
        </div>
        {authenticationTab === AuthenticationTab.LOGIN && (
          <LoginPage onAuthenticationSuccess={onAuthenticationSuccess} />
        )}
        {authenticationTab === AuthenticationTab.SIGN_UP && (
          <SignUpPage onSignUpSuccess={handleSignUpSuccess} />
        )}
        {authenticationTab === AuthenticationTab.RESET_PASSWORD && (
          <ResetPasswordPage onResetSuccess={handleResetSuccess} />
        )}
      </div>
    </div>
  );
